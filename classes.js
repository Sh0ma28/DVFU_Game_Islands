function Blob(x, y, r, isTail){
    ...
    this.eating = function (food) {
        ...
        if (!this.isTail && distance < this.r + food.r ){
            var sum = PI * this.r * this.r + PI * food.r * food.r  // сумма площадей нашей капли и той что сьели
...
function Snake(head){
    ...
    this.show = function(){
        for (var i = 0; i < this.tailLen - 1; i++){
            if (this.tailBlobs[i] != undefined) // Иногда появлсяется ошибка
                this.tailBlobs[i].show();
        }     
    }
}